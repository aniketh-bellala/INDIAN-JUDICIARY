...
<title>Chief Justice Login</title>
...
<?php
  ...
  if(isset($_POST['submit'])){
    ...
    $Judge_search = "SELECT * from Judges where Judge_ID='$JudgeID'";
    $query = mysqli_query($connection,$Judge_search);

    $Judge_count = mysqli_num_rows($query);

    if($Judge_count){
      $Judge_pass = mysqli_fetch_assoc($query);
      $db_pass = $Judge_pass['Password'];

      $_SESSION['admin'] = $Judge_pass['Aadhaar_ID'];

      // $pass_decode = password_verify($Password,$db_pass);

      if($Password==="SC010019" && $JudgeID==="J0019"){
        echo "Login Successfull!";
        header("Location: admin_welcome1.php");
        exit();
      }
      else{
    ...
    <div class="login-form">
        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="judge_id" placeholder="Judge ID" required="required">
            </div>
            <div class="form-group">
