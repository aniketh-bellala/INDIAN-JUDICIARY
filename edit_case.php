...
<body>
    ...
    <div class="content" id="content_id">
        <h3 class="title2">Case Record</h3>
        <h5>Modify Case!</h5>
        <?php
        ...
        $insert = false;
        if (isset($_POST['modify_case_form'])) {
            $cid = $_POST['case_id'];
            if (!empty($cid)) {
                $sql_update1 = "UPDATE case_records SET Final_Verdict = '" . $_POST['verdict'] . "' where Case_ID like '" . $cid . "';";
                $sql_update2 = "UPDATE case_details SET `Statement` = '" . $_POST['statement'] . "' where Case_ID like '" . $cid . "';";
                if (mysqli_query($mysqli, $sql_update1) && mysqli_query($mysqli, $sql_update2)) {
                    $insert = true;
                }
            }
        }
        if ($insert) {
            echo "<p style=\"color:green\">Case Modified!</p>";
        ?>
            <br>
            <a href="./judge_unscheduled.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Go Back!</a>
            <?php
        } else {
            if (isset($_POST['modify_case'])) {
                $ci = $_POST['case_id'];
            ...
            ?>
                <form action="./edit_case.php" method="post">
                    ...
                    <label><strong>Accusor</strong></label>
                    <input type="text" readonly="readonly" value='<?php echo $case_info['Accussor'] ?>'>
                    <label><strong>Accusor Client</strong></label>
                    ...
                    <input type="text" readonly="readonly" name="verdict" value='<?php echo $case_info['Final_Verdict']; ?>'>
                    <label><strong>Statement</strong></label>
                    <input type="text" name="statement" value='<?php echo $case_info['Statement']; ?>'>
                    <input type="submit" class="btn" name="modify_case_form" style="background-color:purple;" onclick="return confirm('Are you sure you want to edit?');">
                </form>
                <a href="./judge_unscheduled.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Go Back!</a>
        <?php
            }
