...
<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if($Judge_count){
      ...
      $_SESSION['judge'] = $Judge_pass['Judge_ID'];

      // $pass_decode = password_verify($Password,$db_pass);

      if($Password === $db_pass){
        echo "Login Successfull!";
        header("Location: judge_welcome1.php");
        exit();
      }
      else{
        header("Location: judge_login.php?err=true");       }
    }
    else{
      header("Location: judge_login.php?err=true");     }
  }
    ...
    <div class="login-form">
        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="judge" placeholder="Judge ID" required="required">
            </div>
            <div class="form-group">
