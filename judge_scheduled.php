...
$sql_pending = "SELECT * from CASE_RECORDS where Final_Verdict like 'Scheduled' and Hearing_By='$_SESSION[judge]';";
...
<body>
    ...
    <div class="content">
        <h3 class="title2">Scheduled Case Records</h3>
        <br>
        <div class="float-left">
        <button type="button" class="btn btn-warning" onclick="location.href='judge_unscheduled.php'">Unscheduled</button>
        <button type="button" class="btn btn-warning" onclick="location.href='judge_scheduled.php'">Scheduled</button>
        <button type="button" class="btn btn-warning" onclick="location.href='judge_past_cases.php'">Past Cases</button>
    </div>
    <br>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td class="cntr"><?php echo $rows["Case_ID"] ?></td>
                        <td class="cntr"><?php echo $rows["Section_Division"] ?></td>
                        <td class="cntr"><?php echo $rows["Hearing_By"] ?></td>
                        <td scope="row" class="row_flex cntr">
                            <form action="./case_info.php" method="post">
                                <?php echo '
                                <input type="hidden" name="case_id" value=\'' . $rows["Case_ID"] . '\'>
                                <button type="submit" class="btn btn-info btn-sm" name="info" onclick="location.href = \'./case_info.php\';">Info</button>'; ?>
                            </form>
                            <form action="./scheduled_case_edit.php" method="post">
                                <?php echo '
                                <input type="hidden" name="case_id" value=\'' . $rows["Case_ID"] . '\'>
                                <button type="submit" class="btn btn-warning btn-sm" name="modify_case" onclick="location.href = \'./scheduled_case_edit.php\';">Edit</button>'; ?>
                            </form>
                            <form action="./delete_case.php" method="post">
                                <?php echo '
                                <input type="hidden" name="case_id" value=\'' . $rows["Case_ID"] . '\'>
                                <button type="submit" class="btn btn-danger btn-sm" name="delete_case" onclick="return confirm(\'Are you sure you want to continue?\');">Delete</button>'; ?>
                            </form>
                        </td>
                    </tr>
                <?php
