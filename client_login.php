...
<?php
  ...
  if(isset($_POST['submit'])){
    $AadhaarID = $_POST['aadhaar'];
    $Password = $_POST['password'];

    $Aadhaar_search = "SELECT * from BIODATA where Aadhaar_ID='$AadhaarID'";
    $query = mysqli_query($connection,$Aadhaar_search);

    $Aadhar_count = mysqli_num_rows($query);

    if($Aadhar_count){
      $Aadhaar_pass = mysqli_fetch_assoc($query);
      $db_pass = $Aadhaar_pass['Password'];

      $_SESSION['aadhaar'] = $Aadhaar_pass['Aadhaar_ID'];

      // $pass_decode = password_verify($Password,$db_pass);

      if($Password===$db_pass){
        echo "Login Successfull!";
        header("Location: client_welcome1.php");
        exit();
      }
      else{
        header("Location: client_login.php?err=true");      }
    }
    else{
      header("Location: client_login.php?err=true");    }
  }
    ...
    <div class="login-form">
        <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="aadhaar" placeholder="Aadhaar ID" maxlength="12" required="required">
            </div>
            <div class="form-group">
